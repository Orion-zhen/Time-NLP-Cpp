...
struct TimeStamp
{
	...
	TimeStamp() {};
	TimeStamp(TimeFormat _formatTime) 
	{
		for (int i = 0; i < 6; i++)
			formatTime[i] = (_formatTime[i] != -1) ? _formatTime[i] : formatTime[i];
		year = formatTime[0];
		month = formatTime[1];
		day = formatTime[2];
		hour = formatTime[3];
...
struct StringPreHandler
{
	string delKeyword(string target, string rules);
	string numberTranslator(string target);
	int wordToNumber(string s);
	int strToInt(string s);
};

struct RangeTimeEnum
{
	int dayBreak = 3;
	int earlyMorning = 8;
	int morning = 10;
...
{
public:
	TimeFormat tunit{ -1, -1, -1, -1, -1, -1 };

	TimePoint() {};
	TimePoint(vector<int> _tunit) : tunit(_tunit) {};
	~TimePoint() {};
};
...
public:
	...
	Lunar() {};
	Lunar(int _lunarYear, int _lunarMonth, int _lunarDay, bool _isleap) :
		lunarYear(_lunarYear), lunarMonth(_lunarMonth), lunarDay(_lunarDay), isleap(_isleap)
	{};
	~Lunar() {};
};
...
public:
	...
	Solar() {};
	Solar(int _solarYear, int _solarMonth, int _solarDay) :
		solarYear(_solarYear), solarMonth(_solarMonth), solarDay(_solarDay)
	{};
	~Solar() {};
};
...
private:
	vector<unsigned int> lunar_month_days
	{
...
public:
	Solar LunarToSolar(const Lunar& lunar);
	Lunar SolarToLunar(const Solar& solar);

	Solar operator() (const Lunar& Lunar) { return LunarToSolar(Lunar); }
	Lunar operator() (const Solar& Solar) { return SolarToLunar(Solar); }
};
...
public:
	...
	string _filter(string input_query);
	void init();
	void __preHandling();
...
public:
	...
	string genSpan(int days, int seconds);
	TimeStamp genTime(const TimeFormat& tunit);
	void norm_setyear();
	void norm_setmonth();
	void norm_setmonth_fuzzyday();
	void norm_setday();
	void norm_checkKeyword();
	void norm_sethour();
	void norm_setminute();
	void norm_setsecond();
	void norm_setSpecial();
	void norm_setBaseRelated();
	void norm_setSpanRelated();
	void norm_setHoliday();
	void norm_setCurRelated();
	void modifyTimeBase();
	void preferFuture(size_t timeIndex);
	TimeStamp preferFutureWeek(int weekday, TimeStamp cur);
	TimeStamp addTime(TimeStamp cur, size_t fore_unit);
	void _check_time(TimeFormat parse);
	void checkContextTime(size_t timeIndex);
};
